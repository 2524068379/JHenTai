...
const String _legacyMouseConfigKey = 'mouse' 'Setting';
const String _legacySuperResolutionConfigKey = 'super' 'Resolution' 'Setting';
...
class AppUpdateService
  ...
  List<UpdateHandler> updateHandlers = [
    ...
    CleanupAndroidOnlyLegacyDataHandler(),
  ];
...
class CleanupAndroidOnlyLegacyDataHandler implements UpdateHandler {
  @override
  List<JHLifeCircleBean> get initDependencies =>
      [localConfigService, storageService];

  @override
  Future<bool> match(int? fromVersion, int toVersion) async {
    return fromVersion == null || fromVersion < toVersion;
  }

  @override
  Future<void> onInit() async {
    log.info('CleanupAndroidOnlyLegacyDataHandler onInit');

    await Future.wait([
      appDb.customStatement(
        "DELETE FROM local_config WHERE config_key IN ('$_legacyMouseConfigKey', '$_legacySuperResolutionConfigKey')",
      ),
      storageService.remove(_legacyMouseConfigKey),
      storageService.remove(_legacySuperResolutionConfigKey),
      storageService.remove('windowWidth'),
      storageService.remove('windowHeight'),
      storageService.remove('windowMaximize'),
      storageService.remove('windowFullScreen'),
      storageService.remove('leftColumnWidthRatio'),
    ]);
  }
