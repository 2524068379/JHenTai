...
abstract class BaseLayout extends StatelessWidget {
  ...
  Size _resolveImageDisplaySize(int index) {
    final Size? cachedSize = logic.readPageState.imageContainerSizes[index];
    if (cachedSize != null) {
      return cachedSize;
    }

    final image = readPageState.images[index];
    if (image?.width != null && image?.height != null) {
      return logic.getImageFittedSizeByGalleryImage(image!).destination;
    }

    return logic.getPlaceHolderSize(index);
  }
  ...
  Widget _buildOnlineImage(BuildContext context, int index) {
    final Size displaySize = _resolveImageDisplaySize(index);
